...
Camera * Camera::msCurrentCamera;

void Camera::Update() {
	mViewMatrix.LookAt( mGlobalTransform.GetOrigin(), mGlobalTransform.GetOrigin() + mGlobalTransform.GetLook(), mGlobalTransform.GetUp() );
	glMatrixMode( GL_MODELVIEW );
	glLoadMatrixf( mViewMatrix.f );


	glMatrixMode( GL_PROJECTION );		
	mProjectionMatrix.PerspectiveFov( (mFov / 2) * Math::Pi / 180.0f, gAspect, 0.05f, 1024.0f );
	glLoadMatrixf( mProjectionMatrix.f );

	mViewProjectionMatrix = mViewMatrix * mProjectionMatrix;

	mInvViewProjMatrix = mViewProjectionMatrix.Inverse();
	
	glMatrixMode( GL_MODELVIEW );

	Math::Vector3 look =  mGlobalTransform.GetLook();
	Math::Vector3 pos = GetGlobalPosition();
	gListener->SetPosition( pos.x, pos.y, pos.z, DS3D_IMMEDIATE );
	gListener->SetOrientation( look.x, look.y, look.z, 0, -1, 0, DS3D_IMMEDIATE );
	ExtractFrustum();
}

Camera::Camera( float fov ) : mFov( fov ) {
	msCurrentCamera = this;
	mType = SceneNode::Type::Camera;
}
